...
add_number = int(input('Введите дополнительное число: '))
...
if not is_int(numbers):
    ...
    print("Программа завершена с ошибкой, начните заново")
else:
    numbers = numbers.split()

list_numbers = [int(pos) for pos in numbers]
...
if not binary_search(list_numbers, add_number, 0, len(list_numbers)):
    rI = min(list_numbers, key=lambda x: (abs(x - add_number), x))
